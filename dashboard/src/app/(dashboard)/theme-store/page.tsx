        ...
        {(purchasedData?.themes || []).length > 0 && (
          <Card className="p-4">
            ...
            <div className="flex gap-3 flex-wrap">
              {(purchasedData?.themes || []).map((pt: any) => (
                <Badge key={pt.id} className="bg-green-100 text-green-700 py-1.5 px-3">
                  {pt.theme?.name || "قالب مُثبَّت"}
                </Badge>
              ))}
            </div>
          </Card>
                  ...
                  <div className="aspect-video bg-gradient-to-br from-gray-100 to-gray-200 relative">
                    ...
                    {isPurchased && (
                      <div className="absolute top-2 right-2">
                        <Badge className="bg-green-500 text-white">مُثبَّت</Badge>
                      </div>
                    )}
                    {theme.isFree && !isPurchased && (
                      <div className="absolute top-2 left-2">
                        <Badge className="bg-blue-500 text-white">مجاني</Badge>
                      </div>
                    )}
                  </div>
                  <div className="p-4">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="text-right shrink-0 mr-2">
                        ...
                        {theme.rating > 0 && (
                          <div className="flex items-center gap-0.5 justify-end text-xs text-yellow-500">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        size="sm"
                        className={`flex-1 ${isPurchased ? "bg-green-500" : "bg-orange-500 hover:bg-orange-600"}`}
                        disabled={isPurchased || purchaseMutation.isPending}
                        onClick={() => !isPurchased && purchaseMutation.mutate(theme.id)}
                      >
                        {isPurchased ? "مُثبَّت" : theme.isFree ? "تثبيت" : "شراء"}
                      </Button>
                    </div>
                  </div>
                </Card>
