...
import { Palette, Save, CheckCircle, ShieldCheck, ShieldOff, QrCode, LayoutTemplate, Users, MessageCircle, Trash2, UserPlus, Send, Phone, Truck, CreditCard } from "lucide-react";
    ...
    <div className="flex flex-col">
      ...
      <div className="p-6">
          ...
          <div className="space-y-6 max-w-3xl">
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          background:
                            t.id === "dark"
                              ? "linear-gradient(135deg, #0f0f1a, #1e1b4b)"
                              : t.id === "elegant"
                              ? `linear-gradient(to bottom, ${t.primaryColor}25, ${t.primaryColor}08)`
                              : t.id === "fresh"
                              ? `linear-gradient(135deg, ${t.primaryColor}55, ${t.secondaryColor}55)`
                              : `linear-gradient(135deg, ${t.primaryColor}, ${t.primaryColor}bb)`,
                        }}
                      >
                        <div
                          className="h-1.5 rounded-full w-3/4"
                          style={{ background: t.id === "elegant" ? t.primaryColor : "rgba(255,255,255,0.8)" }}
                        />
                        <div
                          className="h-1 rounded-full w-2/5"
                          style={{ background: t.id === "elegant" ? t.primaryColor + "60" : "rgba(255,255,255,0.4)" }}
                        />
                      </div>
                      {/* Name + colors */}
            ...
            {/* Appearance */}
            <Card>
              <CardHeader title="المظهر والألوان" />
              <CardBody className="space-y-5">
                <div className="grid gap-6 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div
                        className="flex-1 h-10 rounded-lg border border-slate-200 flex items-center justify-end px-3 text-xs font-mono text-white font-medium"
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div
                        className="flex-1 h-10 rounded-lg border border-slate-200 flex items-center justify-end px-3 text-xs font-mono text-white font-medium"
                        style={{ backgroundColor: secondaryColor ?? "#f97316" }}
                      >
                ...
                <div>
                  ...
                  <select
                    className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:w-64"
                    {...register("fontFamily")}
                  >
                  ...
                  <p className="mt-1 text-xs text-slate-500">الخط المستخدم في متجرك عبر الإنترنت</p>
                </div>
                ...
                <div className="rounded-xl border border-slate-200 overflow-hidden">
                  ...
                  <div className="p-5 flex items-center gap-4 flex-wrap">
                    <button
                      type="button"
                      className="px-5 py-2 rounded-full text-white text-sm font-semibold transition"
                      style={{ backgroundColor: primaryColor ?? "#2563eb" }}
                    >
                      أضف إلى السلة
                    </button>
                    <button
                      ...
                      style={{ borderColor: primaryColor ?? "#2563eb", color: primaryColor ?? "#2563eb" }}
                    >
                      عرض المنتج
                    </button>
                    <span
                      className="text-sm font-medium"
                      style={{ color: secondaryColor ?? "#f97316" }}
                    >
                      عرض خاص
                    </span>
                  </div>
            ...
            {/* 2FA */}
            <Card>
              <CardHeader title="التحقق بخطوتين (2FA)" />
              <CardBody className="space-y-4">
                {twoFASuccess && (
                  <div className="flex items-center gap-2 text-sm font-medium text-emerald-600 bg-emerald-50 border border-emerald-200 rounded-lg px-4 py-3">
                ...
                {!twoFAEnabled && !twoFASetup && (
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5 mb-3">قم بتفعيل 2FA لحماية حسابك أكثر باستخدام تطبيق مثل Google Authenticator أو Authy</p>
                ...
                {!twoFAEnabled && twoFASetup && (
                  <div className="space-y-4">
                    <p className="text-sm text-slate-600">
                      <strong>الخطوة 1:</strong> امسح رمز QR باستخدام تطبيق المصادقة
                    </p>
                    <div className="flex justify-center">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={twoFASetup.qrCode} alt="QR Code" className="w-48 h-48 border rounded-xl p-2" />
                    </div>
                    <p className="text-xs text-center text-slate-500 font-mono bg-slate-50 rounded-lg px-3 py-2 break-all">
                      {twoFASetup.secret}
                    </p>
                    <p className="text-sm text-slate-600">
                      <strong>الخطوة 2:</strong> أدخل الرمز المكون من 6 أرقام للتأكيد
                    </p>
                    <div className="flex gap-3">
                      <input
                        ...
                        placeholder="000000"
                        className="flex-1 text-center text-xl tracking-widest font-mono rounded-lg border border-slate-300 px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      />
                    ...
                    <button type="button" onClick={() => { setTwoFASetup(null); setTwoFACode(""); setTwoFAError(""); }}
                      className="text-sm text-slate-400 hover:text-slate-600">إلغاء</button>
                  </div>
                )}
                ...
                <div className="border border-slate-200 rounded-xl p-4 space-y-3 bg-slate-50">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <input
                      ...
                      className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    />
                    <input
                      type="text"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Button
                      ...
                      disabled={!inviteEmail || !inviteFirst}
                    >
                  ...
                  {inviteMsg && (
                    <p className={`text-sm ${inviteMsg.includes("نجاح") ? "text-emerald-600" : "text-red-500"}`}>
                      {inviteMsg}
                    </p>
                  )}
                </div>
            ...
            <Card>
              <CardHeader title="إشعارات واتساب" />
              <CardBody className="space-y-4">
                <div className="flex items-start gap-3">
                  <MessageCircle className="h-5 w-5 text-green-500 mt-0.5 shrink-0" />
                  <p className="text-sm text-slate-600">
                    أرسل إشعارات تلقائية للعملاء عبر WhatsApp عند تأكيد الطلب أو تحديث حالة الشحن.
                    تأكد من إعداد <strong>WhatsApp Business Cloud API</strong> من Meta.
                  </p>
                </div>
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">إرسال رسائل تأكيد وتحديثات الشحن تلقائياً</p>
                  </div>
                </label>
                  ...
                  <div className="border-t border-slate-100 pt-4 space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500"
                      />
                      <p className="mt-1 text-xs text-slate-500">
                        من Meta Business Suite → WhatsApp → Getting Started
                      </p>
                    </div>
                    <div>
                      <label className="mb-1 block text-sm font-medium text-slate-700">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {waMsg && (
                    <span className={`text-sm ${waMsg.includes("نجاح") ? "text-emerald-600" : "text-red-500"}`}>
                      {waMsg}
                    </span>
                  )}
                </div>
            ...
            {/* SMS Notifications */}
            <Card>
              <CardHeader title="إشعارات SMS (Twilio)" />
              <CardBody className="space-y-4">
                <div className="flex items-start gap-3">
                  <Phone className="h-5 w-5 text-blue-500 mt-0.5 shrink-0" />
                  <p className="text-sm text-slate-600">
                    أرسل رسائل SMS تلقائية للعملاء عند تأكيد الطلب أو تحديث حالة الشحن عبر Twilio.
                  </p>
                </div>
                <label className="flex items-center gap-3 cursor-pointer">
                ...
                {smsEnabled && (
                  <div className="border-t border-slate-100 pt-4 space-y-3">
                    <div>
                      ...
                      <input type="text" placeholder="ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" value={smsSid}
                        onChange={(e) => setSmsSid(e.target.value)}
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                      <label className="mb-1 block text-sm font-medium text-slate-700">Auth Token</label>
                      <input type="password" placeholder="••••••••••••••••" value={smsToken}
                    ...
                    <div>
                      ...
                      <input type="text" placeholder="+12345678901" value={smsFrom}
                ...
                <div className="flex items-center gap-3">
                  <Button type="button" variant="outline" size="sm" onClick={saveSms} loading={smsSaving}>
                    <Save className="h-3.5 w-3.5" />
                    حفظ إعدادات SMS
                  </Button>
            ...
            {/* Aramex Shipping */}
            <Card>
              <CardHeader title="شحن Aramex" />
              <CardBody className="space-y-4">
                <div className="flex items-start gap-3">
                  <Truck className="h-5 w-5 text-orange-500 mt-0.5 shrink-0" />
                  <p className="text-sm text-slate-600">
                    أنشئ شحنات Aramex تلقائياً من لوحة الطلبات وتتبع شحناتك مباشرةً.
                  </p>
                </div>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={aramexEnabled} onChange={(e) => setAramexEnabled(e.target.checked)}
                  ...
                  <div className="border-t border-slate-100 pt-4 space-y-3">
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div>
                        ...
                        <input type="text" value={aramexUser} onChange={(e) => setAramexUser(e.target.value)}
                          className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                      </div>
                      <div>
                        <label className="mb-1 block text-sm font-medium text-slate-700">Password</label>
                        <input type="password" value={aramexPassword} onChange={(e) => setAramexPassword(e.target.value)}
                          className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                      </div>
                    </div>
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div>
                        ...
                        <input type="text" value={aramexAccount} onChange={(e) => setAramexAccount(e.target.value)}
                          className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                      </div>
                      <div>
                        <label className="mb-1 block text-sm font-medium text-slate-700">PIN Code</label>
                        <input type="password" value={aramexPin} onChange={(e) => setAramexPin(e.target.value)}
                          className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-3">
                  <Button type="button" variant="outline" size="sm" onClick={saveAramex} loading={aramexSaving}>
                    <Save className="h-3.5 w-3.5" />
                    حفظ إعدادات Aramex
                  </Button>
            ...
            {/* DHL Shipping */}
            <Card>
              <CardHeader title="شحن DHL Express" />
              <CardBody className="space-y-4">
                <div className="flex items-start gap-3">
                  <Truck className="h-5 w-5 text-yellow-500 mt-0.5 shrink-0" />
                  <p className="text-sm text-slate-600">
                    أنشئ شحنات DHL Express تلقائياً وتتبعها من خلال myDHL+ API.
                  </p>
                </div>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={dhlEnabled} onChange={(e) => setDhlEnabled(e.target.checked)}
                ...
                {dhlEnabled && (
                  <div className="border-t border-slate-100 pt-4 space-y-3">
                    <div>
                      ...
                      <input type="password" value={dhlApiKey} onChange={(e) => setDhlApiKey(e.target.value)}
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-yellow-500" />
                    </div>
                    <div>
                      <label className="mb-1 block text-sm font-medium text-slate-700">Account Number</label>
                ...
                <div className="flex items-center gap-3">
                  <Button type="button" variant="outline" size="sm" onClick={saveDhl} loading={dhlSaving}>
                    <Save className="h-3.5 w-3.5" />
                    حفظ إعدادات DHL
                  </Button>
            ...
            {/* Tabby & Tamara BNPL */}
            <Card>
              <CardHeader title="الدفع بالتقسيط (Tabby & Tamara)" />
              <CardBody className="space-y-6">
                <div className="flex items-start gap-3">
                  <CreditCard className="h-5 w-5 text-indigo-500 mt-0.5 shrink-0" />
                  <p className="text-sm text-slate-600">
                    أتح للعملاء الدفع لاحقاً أو بالتقسيط عبر Tabby وTamara.
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" checked={tabbyEnabled} onChange={(e) => setTabbyEnabled(e.target.checked)}
                      className="h-4 w-4 rounded border-slate-300 text-green-600" />
                    <div>
                      <p className="text-sm font-medium text-slate-900">🟢 تفعيل Tabby</p>
                      <p className="text-xs text-slate-500">ادفع بعد 14 يوم بدون فوائد</p>
                    </div>
                  </label>
                  {tabbyEnabled && (
                    <div className="grid gap-3 sm:grid-cols-2 pr-7">
                      <div>
                        ...
                        <input type="text" value={tabbyPublicKey} onChange={(e) => setTabbyPublicKey(e.target.value)}
                          className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500" />
                      </div>
                      <div>
                        <label className="mb-1 block text-sm font-medium text-slate-700">Secret Key</label>
                        <input type="password" value={tabbySecretKey} onChange={(e) => setTabbySecretKey(e.target.value)}
                          className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500" />
                      </div>
                    </div>
                  )}
                ...
                <div className="space-y-3 border-t border-slate-100 pt-4">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" checked={tamaraEnabled} onChange={(e) => setTamaraEnabled(e.target.checked)}
                  ...
                  {tamaraEnabled && (
                    <div className="pr-7">
                      ...
                      <input type="password" value={tamaraToken} onChange={(e) => setTamaraToken(e.target.value)}
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500" />
                    </div>
                  )}
                </div>

                <div className="flex items-center gap-3">
                  <Button type="button" variant="outline" size="sm" onClick={saveBnpl} loading={bnplSaving}>
                    <Save className="h-3.5 w-3.5" />
                    حفظ إعدادات التقسيط
                  </Button>
            ...
            {/* Apple Pay & Google Pay */}
            <Card>
              <CardHeader title="Apple Pay & Google Pay" />
              <CardBody className="space-y-5">
                <div className="flex items-start gap-3">
                  <CreditCard className="h-5 w-5 text-gray-500 mt-0.5 shrink-0" />
                  <p className="text-sm text-slate-600">
                    أتح للعملاء الدفع السريع عبر Apple Pay وGoogle Pay (يتطلب حساب Stripe أو بوابة دفع تدعم هذه الخيارات).
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {applePayEnabled && (
                    <div className="pr-7">
                      ...
                      <input type="text" value={applePayMerchantId} onChange={(e) => setApplePayMerchantId(e.target.value)}
                        placeholder="merchant.com.yourstore"
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-500" />
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-3 border-t border-slate-100 pt-4">
                  ...
                  {googlePayEnabled && (
                    <div className="pr-7">
                      <label className="mb-1 block text-sm font-medium text-slate-700">Merchant ID</label>
                      <input type="text" value={googlePayMerchantId} onChange={(e) => setGooglePayMerchantId(e.target.value)}
                        placeholder="BCR2DN4XXXXXXXX"
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-3">
                  <Button type="button" variant="outline" size="sm" onClick={saveApg} loading={apgSaving}>
                    <Save className="h-3.5 w-3.5" />
                    حفظ إعدادات Apple/Google Pay
                  </Button>
            ...
            {/* Tap Payments */}
            <Card>
              <CardHeader title="Tap Payments" />
              <CardBody className="space-y-4">
                <div className="flex items-start gap-3">
                  <CreditCard className="h-5 w-5 text-indigo-500 mt-0.5 shrink-0" />
                  <p className="text-sm text-slate-600">
                    بوابة دفع تدعم KNET · Visa · Mastercard · Apple Pay · mada في الكويت والسعودية والبحرين والإمارات.
                    احصل على مفاتيحك من <strong>tap.company</strong>.
                  </p>
                </div>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={tapEnabled} onChange={(e) => setTapEnabled(e.target.checked)}
                    className="h-4 w-4 rounded border-slate-300 text-indigo-600" />
                  <div>
                    <p className="text-sm font-medium text-slate-900">تفعيل Tap Payments</p>
                    <p className="text-xs text-slate-500">يظهر للعملاء خيار الدفع عبر Tap عند الدفع</p>
                  </div>
                </label>
                {tapEnabled && (
                  <div className="border-t border-slate-100 pt-4 space-y-3">
                    <div>
                      <label className="mb-1 block text-sm font-medium text-slate-700">Secret Key</label>
                      <input type="password" value={tapSecretKey} onChange={(e) => setTapSecretKey(e.target.value)}
                        placeholder="sk_live_xxxxxxxxxxxxxxxxxxxx"
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                      <p className="mt-1 text-xs text-slate-500">يُستخدم في الخادم فقط — لا يشاركه مع أحد</p>
                    </div>
                    <div>
                      <label className="mb-1 block text-sm font-medium text-slate-700">Public Key</label>
                      <input type="text" value={tapPublicKey} onChange={(e) => setTapPublicKey(e.target.value)}
                    ...
                    <div className="rounded-lg bg-indigo-50 border border-indigo-100 px-4 py-3 text-xs text-indigo-700">
                      ...
                      <span className="block font-mono mt-1 text-[11px] break-all">
                        {process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001"}/api/v1/payment/tap/webhook
                      </span>
                    </div>
                  </div>
                )}
                <div className="flex items-center gap-3">
                  <Button type="button" variant="outline" size="sm" onClick={saveTap} loading={tapSaving}>
                    <Save className="h-3.5 w-3.5" />
                    حفظ إعدادات Tap
                  </Button>
            ...
            {/* Moyasar */}
            <Card>
              <CardHeader title="Moyasar" />
              <CardBody className="space-y-4">
                <div className="flex items-start gap-3">
                  <CreditCard className="h-5 w-5 text-green-600 mt-0.5 shrink-0" />
                  <p className="text-sm text-slate-600">
                    بوابة دفع سعودية تدعم mada · Visa · Mastercard · Apple Pay · STC Pay.
                    احصل على مفاتيحك من <strong>moyasar.com</strong>.
                  </p>
                </div>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={moyasarEnabled} onChange={(e) => setMoyasarEnabled(e.target.checked)}
                    className="h-4 w-4 rounded border-slate-300 text-green-600" />
                  <div>
                    <p className="text-sm font-medium text-slate-900">تفعيل Moyasar</p>
                    <p className="text-xs text-slate-500">مثالي للمتاجر السعودية — يدعم mada وSTC Pay</p>
                  </div>
                </label>
                {moyasarEnabled && (
                  <div className="border-t border-slate-100 pt-4 space-y-3">
                    <div>
                      ...
                      <input type="password" value={moyasarSecretKey} onChange={(e) => setMoyasarSecretKey(e.target.value)}
                        placeholder="sk_live_xxxxxxxxxxxxxxxxxxxx"
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500" />
                    </div>
                    <div>
                      <label className="mb-1 block text-sm font-medium text-slate-700">Publishable API Key</label>
                      <input type="text" value={moyasarPublicKey} onChange={(e) => setMoyasarPublicKey(e.target.value)}
                        placeholder="pk_live_xxxxxxxxxxxxxxxxxxxx"
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500" />
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-3">
                  <Button type="button" variant="outline" size="sm" onClick={saveMoyasar} loading={moyasarSaving}>
                    <Save className="h-3.5 w-3.5" />
                    حفظ إعدادات Moyasar
                  </Button>
            ...
            {/* Benefit Pay */}
            <Card>
              <CardHeader title="BenefitPay (البحرين)" />
              <CardBody className="space-y-4">
                <div className="flex items-start gap-3">
                  <CreditCard className="h-5 w-5 text-red-500 mt-0.5 shrink-0" />
                  <p className="text-sm text-slate-600">
                    خدمة الدفع الرسمية بالبحرين عبر شبكة Benefit.
                    تواصل مع بنكك للحصول على بيانات التكامل.
                  </p>
                </div>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={benefitEnabled} onChange={(e) => setBenefitEnabled(e.target.checked)}
                    className="h-4 w-4 rounded border-slate-300 text-red-600" />
                  <div>
                    <p className="text-sm font-medium text-slate-900">تفعيل BenefitPay</p>
                    <p className="text-xs text-slate-500">دفع مباشر من الحساب البنكي البحريني</p>
                  </div>
                </label>
                {benefitEnabled && (
                  <div className="border-t border-slate-100 pt-4 space-y-3">
                    <div>
                      <label className="mb-1 block text-sm font-medium text-slate-700">Merchant ID</label>
                      <input type="text" value={benefitMerchantId} onChange={(e) => setBenefitMerchantId(e.target.value)}
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500" />
                    </div>
                    <div>
                      <label className="mb-1 block text-sm font-medium text-slate-700">API Key</label>
                      <input type="password" value={benefitApiKey} onChange={(e) => setBenefitApiKey(e.target.value)}
                        className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-mono placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-red-500" />
                    </div>
                  </div>
                )}
                <div className="flex items-center gap-3">
                  <Button type="button" variant="outline" size="sm" onClick={saveBenefit} loading={benefitSaving}>
                    <Save className="h-3.5 w-3.5" />
                    حفظ إعدادات BenefitPay
                  </Button>
            ...
            <Card>
              <CardHeader
                title="نقاط الولاء"
                subtitle="كافئ عملاءك بنقاط قابلة للاستبدال"
              />
              <CardBody className="space-y-4">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={loyaltyEnabled} onChange={(e) => setLoyaltyEnabled(e.target.checked)}
                    className="h-4 w-4 rounded accent-indigo-600" />
                  <span className="text-sm font-medium text-slate-700">تفعيل برنامج الولاء</span>
                </label>
                {loyaltyEnabled && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                    <div>
                      <label className="text-xs font-medium text-slate-600 mb-1 block">نقاط لكل 1 دينار بحريني</label>
                      <input type="number" min={1} value={loyaltyPointsPerBD} onChange={(e) => setLoyaltyPointsPerBD(Number(e.target.value))}
                        className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" />
                    </div>
                    <div>
                      <label className="text-xs font-medium text-slate-600 mb-1 block">قيمة النقطة الواحدة (دينار)</label>
                      <input type="number" min={0.0001} step={0.001} value={loyaltyBDPerPoint} onChange={(e) => setLoyaltyBDPerPoint(Number(e.target.value))}
                        className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" />
                    </div>
                    <div>
                      <label className="text-xs font-medium text-slate-600 mb-1 block">الحد الأدنى للاستخدام (نقطة)</label>
                      <input type="number" min={1} value={loyaltyMinRedeem} onChange={(e) => setLoyaltyMinRedeem(Number(e.target.value))}
                        className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" />
                    </div>
                    <div>
                      <label className="text-xs font-medium text-slate-600 mb-1 block">أقصى خصم بالنقاط (% من الطلب)</label>
                      <input type="number" min={1} max={100} value={loyaltyMaxRedeemPct} onChange={(e) => setLoyaltyMaxRedeemPct(Number(e.target.value))}
                        className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" />
                    </div>
                  </div>
                )}
                <p className="text-xs text-slate-400">
                  {loyaltyEnabled
                    ? `العميل يكسب ${loyaltyPointsPerBD} نقطة لكل دينار، وكل ${Math.round(1 / loyaltyBDPerPoint)} نقطة = 1 دينار خصم`
                    : "فعّل البرنامج لمكافأة عملائك الأوفياء"}
                </p>
                <div className="flex items-center gap-3">
                  <Button type="button" variant="outline" size="sm" onClick={saveLoyalty} loading={loyaltySaving}>
                    <Save className="h-3.5 w-3.5" />
                    حفظ إعدادات الولاء
                  </Button>
