...
export default function LiveCommercePage() {
  ...
  const startMutation = useMutation({
    mutationFn: async (id: string) => api.post(`/live/streams/${id}/start`, {}),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["live-streams"] }),
  });

  const endMutation = useMutation({
    mutationFn: async (id: string) => api.post(`/live/streams/${id}/end`, {}),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["live-streams"] }),
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Header
        title="البث المباشر"
        subtitle="بيع منتجاتك عبر البث المباشر"
        action={
          <Button onClick={() => setShowForm(true)} className="flex items-center gap-2">
            <Plus className="w-4 h-4" />
            جلسة بث جديدة
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-5">
          ...
          <Card className="p-5 bg-red-50 border-red-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-bold text-red-800">{liveStream.title}</div>
                  <div className="flex items-center gap-3 text-sm text-red-600 mt-1">
                    <span className="flex items-center gap-1"><Eye className="w-3 h-3" />{liveStream.viewerCount || 0} مشاهد</span>
                    <span className="flex items-center gap-1"><ShoppingBag className="w-3 h-3" />{liveStream.orderCount || 0} طلب</span>
                  </div>
                </div>
              </div>
              <Button
                variant="danger"
                size="sm"
                onClick={() => endMutation.mutate(liveStream.id)}
              >
                <Square className="w-4 h-4 mr-1" />
                إنهاء البث
              </Button>
            ...
            {liveStream.rtmpKey && (
              <div className="mt-3 bg-red-100 rounded-lg p-3">
                <div className="text-xs text-red-600 mb-1">RTMP URL للبث</div>
                <code className="text-xs text-red-800 font-mono">rtmp://live.bazar.com/app/{liveStream.rtmpKey}</code>
              </div>
            )}
          </Card>
        )}
        ...
        {showForm && (
          <Card className="p-5">
            <h3 className="font-semibold mb-4">إنشاء جلسة بث</h3>
            <div className="space-y-3">
              <Input placeholder="عنوان البث" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} />
            ...
            <div className="flex gap-2 mt-4">
              <Button onClick={() => createMutation.mutate()} disabled={!form.title || createMutation.isPending}>إنشاء</Button>
              <Button variant="outline" onClick={() => setShowForm(false)}>إلغاء</Button>
            </div>
          </Card>
        )}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-400 mt-0.5">
                          <span>{formatDate(s.createdAt)}</span>
                          {s.scheduledAt && <span>مجدول: {formatDate(s.scheduledAt)}</span>}
                          <span className="flex items-center gap-1"><Eye className="w-3 h-3" />{s.viewerCount || 0}</span>
                          <span className="flex items-center gap-1"><ShoppingBag className="w-3 h-3" />{s._count?.products || 0} منتج</span>
                        </div>
                      </div>
        ...
        {!isLoading && streams.length === 0 && (
          <div className="text-center py-12">
            <Video className="w-12 h-12 text-gray-300 mx-auto mb-3" />
            <p className="text-gray-500">لم يتم إنشاء أي جلسة بث مباشر</p>
            <Button className="mt-4" onClick={() => setShowForm(true)}>
              <Plus className="w-4 h-4 mr-2" />
              أنشئ أول جلسة بث
            </Button>
