...
async function callOpenAI(
  ...
  if (!OPENAI_KEY) {
    // Fallback: generate template response for demo/dev
...
// ─── Fallback: template-based responses when no API key ──────────────────────
function generateFallbackResponse(prompt: string): string {
  const lowerPrompt = prompt.toLowerCase()
  if (lowerPrompt.includes('وصف') || lowerPrompt.includes('description')) {
    return 'منتج عالي الجودة مصنوع بأفضل المواد. يتميز بتصميم أنيق وأداء متميز يناسب احتياجاتك اليومية. يأتي مع ضمان الجودة وخدمة ما بعد البيع.'
  }
  if (lowerPrompt.includes('سعر') || lowerPrompt.includes('price')) {
    return JSON.stringify({ suggestedPrice: 25.9, minPrice: 18, maxPrice: 35, confidence: 0.78, reasoning: 'بناءً على المنتجات المشابهة في السوق' })
  }
  return 'يمكنني مساعدتك في تطوير متجرك وزيادة مبيعاتك. أخبرني بما تحتاجه!'
}
...
export async function aiRoutes(app: FastifyInstance) {
  ...
  app.post('/product-writer', { preHandler: authenticate }, async (request, reply) => {
    ...
    try {
      ...
      return reply.send({ success: true, data: result, model: OPENAI_KEY ? 'gpt-4o' : 'template' })
    } catch (err: any) {
      return reply.status(500).send({ error: 'فشل توليد المحتوى', details: err?.message })
    }
  })
...
أجب بشكل موجز ومفيد. قدم توصيات عملية قابلة للتنفيذ.`
    ...
    try {
      ...
      return reply.send({ success: true, response, model: OPENAI_KEY ? 'gpt-4o' : 'template' })
    } catch (err: any) {
      return reply.status(500).send({ error: 'فشل المساعد الذكي', details: err?.message })
    }
  })
...
اقترح السعر الأمثل. رد بـ JSON: { suggestedPrice, minPrice, maxPrice, marginPercent, reasoning, competitivePosition }`
    ...
    try {
      ...
      return reply.send({ success: true, data: result, similarProducts: similar.length })
    } catch (err: any) {
      return reply.status(500).send({ error: 'فشل اقتراح السعر', details: err?.message })
    }
  })
...
رد بـ JSON: { summary, insights: [{ title, description, priority, impact }] }`
    ...
    try {
      ...
      return reply.status(500).send({ error: 'فشل تحليل المتجر', details: err?.message })
    }
  })
